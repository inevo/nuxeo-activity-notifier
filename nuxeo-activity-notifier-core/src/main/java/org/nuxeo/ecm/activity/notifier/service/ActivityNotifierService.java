...
public class ActivityNotifierService extends DefaultComponent implements
		ActivityNotifierManager {
	private static final Log log = LogFactory
			.getLog(ActivityNotifierService.class);
	...
	public void activate(ComponentContext context) throws Exception {
		...
		// settings.documentRelation = RelationshipKind.newInstance("document",
		// "followes").toString();
		// settings.userRelation = RelationshipKind.newInstance("circle",
		// "followes").toString();
			...
			for (Object contrib : contribs) {
				try {
					...
					notificationRegistry.registerNotification(notifDesc,
							notifDesc.getRelationshipKindDescriptors());
				} catch (Exception e) {
					log.error(e);
				}
			}
	...
	protected void registerSettings(SettingsDescriptor desc) {
		...
		// generalSettings.eMailSubjectPrefix =
		// Framework.expandVars(generalSettings.eMailSubjectPrefix);
	}
